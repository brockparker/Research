...
path = 'wiro_third'
date = 2459368
name = 'wiro_white_final.pdf'
title = 'HD-189733b 2021-06-02 WIRO White Light Analysis'#\nRosenthal Priors and Detrending'
detrend = 0
#########################################################################################
...
for k in range(4):
	transit_data = pd.read_csv(r'/d/tel2/brock/Data/RBO/hd189733/20210602/exofast/' + path + '/hd189733.mcmc.model.transit_' + str(k) + '.txt', sep=" ", header=None)
	...
	model_data = pd.read_csv(r'/d/tel2/brock/Data/RBO/hd189733/20210602/exofast/' + path + '/hd189733.mcmc.detrendedmodel.transit_' + str(k) + '.planet_0.txt', sep=" ", header=None)
	df_model = pd.DataFrame(model_data)
	time_model = np.array(df_model[0]) - date
	light_model = np.array(df_model[1]) + 1


	pretty_data = pd.read_csv(r'/d/tel2/brock/Data/RBO/hd189733/20210602/exofast/' + path + '/hd189733.mcmc.prettymodel.transit_' + str(k) + '.planet_0.txt', sep=" ", header=None)
	df_pretty = pd.DataFrame(pretty_data)
	time_pretty = np.array(df_pretty[0]) - date
	light_pretty = np.array(df_pretty[1]) + 1


	residual_data = pd.read_csv(r'/d/tel2/brock/Data/RBO/hd189733/20210602/exofast/' + path + '/hd189733.mcmc.residuals.transit_' + str(k) + '.txt', sep=" ", header=None)
